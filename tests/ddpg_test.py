...
for _ in range(episode_number):
    print("hellooooo")
    env = robot_env() # initialize environment
    ...
    env.time = 0
    env.start_k = [env.k1, env.k2] # save starting ks
    ...
    for step in range(N): # or while True:
        ...
        state, reward, done, info = env.reward_calculation(action[0]) # reward is du-1 - du
        ...
        storage['k']['k1'].append(env.k1)
        storage['k']['k2'].append(env.k2)
...
env.visualization(storage['pos']['x'],storage['pos']['y'],storage['pos']['z'])
